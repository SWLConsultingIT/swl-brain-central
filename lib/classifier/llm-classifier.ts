...
export async function llmClassify(
  ...
  const systemPrompt = [
    ...
    `---`,
    ``,
    `HARD EXCLUSIONS: NONE on domain or scope grounds — evaluate every job on fit with the 8 business units. If scope fits, match=true.`,
    ``,
    `COACHING / TEACHING EXCLUSION: SWL DOES the work; it does NOT teach others to do it. Set match=false ONLY when the job is ENTIRELY or PREDOMINANTLY coaching/teaching — the CORE deliverable is knowledge transfer to the client: coaching, mentoring, tutoring, training the client/their team, lessons/courses, "teach me / train us / show us how to", or advisory sessions whose whole goal is to EXPLAIN/walk the client through how a tool works so THEY can do it ("explain how X works", "walk us through", "workshop to understand Y"). KEEP (match=true) jobs where SWL does real work (build, implementation, management, audit, strategy) EVEN IF they include SOME training, a handoff, documentation, or a kickoff call — a minor teaching component does NOT make it a coaching job. Test: is teaching the WHOLE point (exclude) or a side part of real delivery (keep)? When excluded, reason MUST start with "Coaching/teaching:".`,
    ``,
    `MINIMUM SCOPE (~20 HOURS): SWL does not take tiny one-off jobs. Set match=false when the DESCRIPTION clearly indicates the total work is small / under ~20 hours: explicit small estimates ("2-6 hours", "a few hours", "~5 hrs"), "quick task", "one-time small fix", a single short call/workshop, or an obviously brief one-off. KEEP jobs that describe substantial, multi-week, ongoing/retainer, or larger scope. If the description gives NO clear size/duration signal, DO NOT discard on this basis — judge on fit. When excluded for this, reason MUST start with "Scope <20h:".`,
    ``,
    `NO-AGENCY EXCLUSION: SWL is an agency. Set match=false when the description EXPLICITLY wants ONLY an individual freelancer and NOT agencies — e.g. "no agencies", "agencies please do not apply", "individuals only", "freelancers only, no agencies", "not looking for an agency/company". Do NOT exclude jobs that merely use the word "freelancer" (all Upwork jobs are freelance) or that say nothing about agencies. When excluded, reason MUST start with "No agencies:".`,
    ``,
