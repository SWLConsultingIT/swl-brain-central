...
export async function llmClassify(
  ...
  const systemPrompt = [
    ...
    ``,
    `COACHING / TEACHING EXCLUSION (the ONE exception): SWL DOES the work; it does NOT teach others to do it. Set match=false when the job's PRIMARY deliverable is knowledge transfer to the client instead of SWL performing the work — coaching, mentoring, tutoring, training the client/their team, lessons/courses, "teach me / train us / show us how to", OR advisory sessions whose goal is to EXPLAIN or walk the client through how a tool/process works so THEY can do it themselves ("explain how X works", "walk us through", "workshop to understand Y"). When excluded, reason MUST start with "Coaching/teaching:". NOT excluded: SWL doing the actual build, implementation, management, audit, or strategic deliverable. Test: does SWL DELIVER the outcome (match=true) or TEACH the client to deliver it (match=false)?`,
    ``,
